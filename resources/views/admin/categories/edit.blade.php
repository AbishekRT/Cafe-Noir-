...
--}}
<x-app-layout>
    <x-slot name="title">Edit {{ $category->name }} - Admin - {{ config('cafe.name') }}</x-slot>
    ...
    <div class="min-h-screen bg-gray-100">
        ...
        <div class="bg-primary shadow">
            <div class="container mx-auto px-4 py-6">
                ...
                <h1 class="text-2xl font-heading font-bold text-secondary">Edit Category</h1>
            </div>
        </div>
                ...
                <form action="{{ route('admin.categories.update', $category) }}" method="POST"
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-accent focus:border-accent resize-none @error('description') border-red-500 @enderror">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    @if($category->image)
                        <div>
                            <label class="block text-sm font-medium text-heading mb-2">Current Image</label>
                            <div class="flex items-center space-x-4">
                                <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}"
                                    class="w-32 h-20 object-cover rounded-lg">
                                <label class="flex items-center text-sm text-red-600 cursor-pointer">
                                    <input type="checkbox" name="remove_image" value="1"
                                        class="w-4 h-4 text-red-600 border-gray-300 rounded focus:ring-red-600 mr-2">
                    ...
                    <div>
                        ...
                        <div
                            class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-accent transition-colors">
                    ...
                    <div class="flex items-center justify-end space-x-4 pt-4 border-t border-gray-200">
                        ...
                        <button type="submit"
                            class="px-6 py-3 bg-accent text-primary font-medium rounded-lg hover:bg-accent/90 transition-colors">
                            Update Category
                        </button>
                    </div>
                </form>
